...
const LessonsPage = async () => {
	...
	const groupedLessons = (lessons ?? []).reduce(
		...
		{} as Record<
			number,
			{
				course: { id: number; title: string | null; slug: string | null }
				modules: Record<
					number,
					{
		...
		<div className='space-y-6'>
			...
			{Object.keys(groupedLessons).length === 0 ? (
				...
				<div className='space-y-8'>
					{Object.values(groupedLessons).map(({ course, modules }) => (
						<div
							key={course.id}
							className='space-y-4'
						>
							...
							{Object.values(modules).map(({ module, lessons: moduleLessons }) => (
								<div
									key={module.id}
