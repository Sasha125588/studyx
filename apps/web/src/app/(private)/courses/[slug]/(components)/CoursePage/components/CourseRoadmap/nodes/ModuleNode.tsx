...
const ModuleNode = memo(({ data }: NodeProps<ModuleNodeType>) => {
	...
	return (
		<div
			className={`h-full w-full rounded-2xl border-2 border-dashed border-violet-500/40 bg-violet-950/20 p-4 backdrop-blur-sm`}
		>
			<div className='flex items-center justify-between gap-4'>
				<div className='flex items-center gap-2'>
					<span className='text-xs font-medium text-violet-400'>Модуль {moduleNumber}</span>
					<span className='text-violet-500/60'>•</span>
					<span className='text-muted-foreground text-xs'>
						{completedCount}/{lessonsCount} уроків
					</span>
				</div>
				{progress > 0 && (
