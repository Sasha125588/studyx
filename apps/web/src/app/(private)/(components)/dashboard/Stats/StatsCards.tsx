	...
	return (
		<div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
			{statsCards.map((card, index) => (
				<Card
					key={card.title}
					className='relative overflow-hidden shadow-xs transition duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-sm'
				>
					...
					<CardContent className='relative space-y-4'>
						<div className='flex items-center justify-between'>
							<div className='flex items-center gap-3'>
								...
								<p className='text-base leading-tight font-semibold'>{card.title}</p>
							</div>
							{card.trend && (
								<div
						...
						<div className='space-y-1'>
							<p className='text-3xl leading-none font-extrabold tracking-tight'>{card.value}</p>
