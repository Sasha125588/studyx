...
function getBaseURL() {
  if (IS_VERCEL) {
    return process.env.FRONTEND_VERCEL_URL || process.env.NEXT_PUBLIC_FRONTEND_VERCEL_URL || ''
  }
  return 'http://localhost:3024'
}

// Fixed version of @siberiacancode/fetches without TS4094 errors
...
// Fixed interface without extending Fetches to avoid TS4094 errors
interface Instance {
  readonly baseURL: BaseUrl
  headers: Record<string, string>
  parse?: ResponseParse
  ...
  setParse: (parse: ResponseParse) => void
...
// Define typed API interface for proper generic support
interface TypedApi {
  get: <T = any>(endpoint: string, options?: Omit<RequestOptions, 'body'>) => Promise<FetchesResponse<T>>
  post: <T = any>(endpoint: string, body?: RequestBody, options?: RequestOptions) => Promise<FetchesResponse<T>>
  put: <T = any>(endpoint: string, body?: RequestBody, options?: RequestOptions) => Promise<FetchesResponse<T>>
  patch: <T = any>(endpoint: string, body?: RequestBody, options?: RequestOptions) => Promise<FetchesResponse<T>>
  delete: <T = any>(endpoint: string, options?: Omit<RequestOptions, 'body'>) => Promise<FetchesResponse<T>>
}

const fetches = new Fetches() as any as Instance;
(fetches as any).Fetches = Fetches;
(fetches as any).create = (params?: FetchesParams) => new Fetches(params)

export const api = (fetches as any).create({
  baseURL: getBaseURL(),
}) as TypedApi
